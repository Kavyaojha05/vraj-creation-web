  ...
  return (
    <section className="relative h-[100svh] min-h-[580px] w-full overflow-hidden bg-black">
              ...
              <div className="mx-auto w-full max-w-7xl px-6 sm:px-10 lg:px-12">
                <div className="max-w-2xl text-left">
                  ...
                  <div className="mb-3.5 flex flex-col gap-1">
                    <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#e5b567] drop-shadow-sm sm:text-xs">
                      ✦ {slide.tag}
                    </span>
                    {slide.tagSub && (
                      <span className="text-[10px] font-medium tracking-[0.18em] text-[#f0dfcd]/80 sm:text-[11px]">
                        {slide.tagSub}
                      </span>
                    )}
                  </div>
                  ...
                  {/* Headline */}
                  <h1 className="mb-4 text-3xl font-extrabold leading-tight text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] sm:text-5xl lg:text-6xl">
                    <span className="block">{slide.title}</span>
                    <span className="mt-1 block bg-gradient-to-r from-[#ffd89b] to-[#e5a054] bg-clip-text text-transparent">
                      {slide.highlight}
                    </span>
                  </h1>

                  {/* Description */}
                  <p className="mb-7 max-w-xl text-sm leading-relaxed text-[#f0dfcd] drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] sm:text-base">
                    {slide.desc}
                  </p>
      ...
      <button
        ...
        className="absolute left-6 top-1/2 z-30 hidden -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-black/35 p-2.5 text-white backdrop-blur-sm transition-all hover:bg-[#8f3424] sm:flex"
      >
      ...
      <button
        ...
        className="absolute right-6 top-1/2 z-30 hidden -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-black/35 p-2.5 text-white backdrop-blur-sm transition-all hover:bg-[#8f3424] sm:flex"
      >
        <FiChevronRight size={20} />
      </button>

      {/* Slide Indicators */}
      <div className="absolute bottom-8 left-6 right-6 z-30 flex items-center justify-between sm:left-12 sm:right-12">
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                active === index ? "w-8 bg-[#e5b567]" : "w-3 bg-white/40"
              }`}
            />
          ))}
        </div>
